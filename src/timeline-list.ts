...
export async function getListsSubscribers(
  listsId: string,
  maxItems: number,
  auth: TwitterAuth,
): Promise<QueryProfilesResponse> {
  const timeline = await getListsTimeline(
    listsId,
    maxItems,
    QueryListsMode.ListSubscribers,
    auth,
  );
  return parseListsTimelineSubscribers(timeline);
}
...
export async function getListsTimeline(
  ...
  switch (queryMode) {
    ...
    case QueryListsMode.TweetsList:
      ...
      break;
    case QueryListsMode.ListSubscribers:
      queryUrl = `https://x.com/i/api/graphql/3BciWYztnlAEGjnpGzs0AA/ListSubscribers`;
      break;
    default:
  ...
  if (
    queryMode === QueryListsMode.ListSubscribers ||
    queryMode === QueryListsMode.UsersList
  ) {
    features = {
      ...listFeatures,
      rweb_video_screen_enabled: false,
...
export function parseListsTimelineSubscribers(
  timeline: ListTimeline,
): QueryProfilesResponse {
  let bottomCursor: string | undefined;
  let topCursor: string | undefined;
  const profiles: Profile[] = [];
  const instructions =
    timeline.data?.list?.subscribers_timeline?.timeline?.instructions ?? [];
  for (const instruction of instructions) {
    if (
      instruction.type === 'TimelineAddEntries' ||
      instruction.type === 'TimelineReplaceEntry'
